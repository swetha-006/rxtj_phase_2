...
#
# FIXES: Loads all Phase 1 artifacts directly via joblib/torch
# (no preprocessing.py import — avoids function name mismatches)
# ============================================================
...
# ════════════════════════════════════════════════════════════
# STEP 1 — Generate real Phase 1 risk scores
# ════════════════════════════════════════════════════════════
print("\n[STEP 1] Loading Phase 1 preprocessors directly...")
...
# ── Phase 1 model classes (copy from app.py) ─────────────────────────────────
...
class AttentionRXTJ(nn.Module):
    def __init__(self, input_dim=50, seq_len=8):
        super().__init__()
        ...
        self.resnext = nn.Sequential(
            ...
            ResNeXtBlock(128, 64),
            ResNeXtBlock(64, seq_len * feat_dim),
        )
        self.gru_attn = SelfAttentionGRU(
            input_size=feat_dim, hidden_size=64, num_layers=2)
        self.fc = nn.Linear(64, 1)
    def forward(self, x):
        feat     = self.resnext(x)
        b        = feat.size(0)
        feat_dim = feat.size(1) // self.seq_len
        seq      = feat.view(b, self.seq_len, feat_dim)
        ctx, aw  = self.gru_attn(seq)
        return torch.sigmoid(self.fc(ctx).squeeze(1)), aw

p1_model = AttentionRXTJ(input_dim=IPCA_DIM, seq_len=8).to(DEVICE)
p1_model.load_state_dict(
    torch.load(os.path.join(MODEL_DIR, "attention_rxtj.pt"), map_location=DEVICE))
p1_model.eval()
print(f"  attention_rxtj.pt loaded")
...
print("\n[STEP 1] Building feature matrix from snapshot...")
...
# Score
print("[STEP 1] Scoring ensemble (AttentionRXTJ + IFM)...")
BATCH    = 2048
all_risk = []
t0       = time.time()
...
print(f"  Predicted fraud rate @ threshold: "
...
print(f"  P1 col range: {X[:,7].min():.4f}–{X[:,7].max():.4f}  "
      f"std={X[:,7].std():.4f}")
...
best_auc, best_st, patience_cnt = 0.0, None, 0
t_tr = time.time()
...
for ep in range(1, 201):
    ...
    for xb, yb in loader:
        ...
        opt.zero_grad()
        l = criterion(logits(model_f, xb), yb)
        l.backward()
    ...
    model_f.eval()
    with torch.no_grad():
        vp = torch.sigmoid(logits(model_f, Xv_t)).cpu().numpy()
...
with torch.no_grad():
    vp_np = torch.sigmoid(logits(model_f, Xv_t)).cpu().numpy()
...
with torch.no_grad():
    tp_p  = torch.sigmoid(logits(model_f, Xt_t)).cpu().numpy()
