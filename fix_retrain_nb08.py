...
#
# Full EARN+ pipeline (verified from NB03 + model weight shapes):
#   224 → Autoencoder encoder → 64-dim latent
#   224 → ResNet (blocks, no head) → 64-dim features
#   concat(64, 64) → 128
#   Nystroem(128 → 300) → IPCA(300 → 50)
#   → AttentionRXTJ(50) + IFM → ensemble risk score
# ============================================================
...
# ════════════════════════════════════════════════════════════
# MODEL CLASSES — exact copies from app.py + NB03
# ════════════════════════════════════════════════════════════

SEQ_LEN     = 8
CARDINALITY = 4

# ── AttentionRXTJ (from app.py) ───────────────────────────────────────────────
...
# ── EARN+ feature extractors (from NB03) ─────────────────────────────────────

class FraudAutoencoder(nn.Module):
    """Autoencoder from NB03. Only encoder is used for feature extraction."""
...
class FraudResNet(nn.Module):
    """ResNet feature extractor from NB03. Output = 64-dim (before head)."""
    def __init__(self, input_dim=224, out_dim=64):
        super().__init__()
        ...
        self.head = nn.Linear(64, 2)   # classifier head (not used for features)
    def extract(self, x):
        """Return 64-dim features (bypass head)."""
...
print(f"  nystroem : expects {nystroem.n_features_in_} features (= 64+64 concat)")
print(f"  ipca     : {IPCA_DIM} components")

# Load autoencoder
ae_state = torch.load(os.path.join(MODEL_DIR, "autoencoder.pt"), map_location=DEVICE)
ae_in    = ae_state["encoder.0.weight"].shape[1]   # 224
ae_model = FraudAutoencoder(input_dim=ae_in, latent_dim=64)
ae_model.load_state_dict(ae_state)
ae_model.eval()
print(f"  autoencoder loaded (input={ae_in} → latent=64)")

# Load ResNet extractor
rn_state = torch.load(os.path.join(MODEL_DIR, "resnet_extractor.pt"), map_location=DEVICE)
rn_model = FraudResNet(input_dim=ae_in, out_dim=64)
rn_model.load_state_dict(rn_state)
rn_model.eval()
print(f"  resnet_extractor loaded (input={ae_in} → features=64)")
...
p1_model.eval()
print(f"  attention_rxtj.pt loaded (input={IPCA_DIM})")
...
# ════════════════════════════════════════════════════════════
# STEP 2 — Build EARN+ features: 224 → 128
# ════════════════════════════════════════════════════════════
print("\n[STEP 2] Building EARN+ feature matrix from snapshot...")
...
# Extract AE latents + ResNet features in batches → concat → 128
print("[STEP 2] Extracting EARN+ features (AE + ResNet, ~3 min)...")
BATCH      = 4096
Z_ae_parts = []
Z_rn_parts = []
t0         = time.time()
...
with torch.no_grad():
    for s in range(0, len(X_sc), BATCH):
        ...
        # Autoencoder encoder → 64
        z_ae = ae_model.encode(xb).cpu().numpy()
        # ResNet extractor → 64 (bypass head)
        z_rn = rn_model.extract(xb).cpu().numpy()
        Z_ae_parts.append(z_ae)
        Z_rn_parts.append(z_rn)
        if (s // BATCH) % 25 == 0:
            print(f"  AE+ResNet: {e:>7,}/{len(X_sc):,}  ({time.time()-t0:.0f}s)")

Z_ae   = np.vstack(Z_ae_parts).astype(np.float32)
Z_rn   = np.vstack(Z_rn_parts).astype(np.float32)
Z_earn = np.hstack([Z_ae, Z_rn]).astype(np.float32)   # (N, 128)
print(f"\n  Z_earn shape: {Z_earn.shape}  ← must be (590540, 128)")
assert Z_earn.shape[1] == nystroem.n_features_in_, \
    f"EARN dim {Z_earn.shape[1]} != nystroem expects {nystroem.n_features_in_}"
print(f"  ✓ Dimension check passed: {Z_earn.shape[1]} == {nystroem.n_features_in_}")

# ════════════════════════════════════════════════════════════
# STEP 3 — Nystroem → IPCA → AttentionRXTJ + IFM scoring
# ════════════════════════════════════════════════════════════
print("\n[STEP 3] Nystroem + IPCA in chunks (~5 min)...")
CHUNK  = 8000
...
print(f"  IPCA done in {time.time()-t0:.0f}s. Shape: {X_ipca.shape}")

print("\n[STEP 3] Scoring with AttentionRXTJ + IFM ensemble...")
all_risk = []
t0       = time.time()
with torch.no_grad():
...
if p1_scores.std() < 0.005:
    print("  ERROR: std near zero — scoring failed. Check model loading.")
    sys.exit(1)
...
# ════════════════════════════════════════════════════════════
# STEP 4 — Retrain FusionNet v2
# ════════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("  STEP 4: Retraining FusionNet v2")
print("=" * 60)
...
print(f"  P1 col: {X[:,7].min():.4f}–{X[:,7].max():.4f}  std={X[:,7].std():.4f}  ← must be > 0.01")
...
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR,"fusion_roc_v2.png"), dpi=150, bbox_inches="tight")
plt.close()

torch.save(model_f.state_dict(), os.path.join(MODEL_DIR,"fusion_net.pt"))
cfg_out = {
    "input_dim": X_tr.shape[1],
    "fusion_threshold": round(OPT_T,6), "high_threshold": round(HIGH_T,6),
    "elevated_threshold": round(ELEV_T,6),
    "fusion_auc": round(float(auc),6), "fusion_mcc": round(float(mcc),6),
    "fusion_precision": round(float(prec),6), "fusion_recall": round(float(rec),6),
...
print(f"\n  ✓ fusion_net.pt + fusion_config.json + fusion_scaler.pkl saved")
print(f"\n{'='*60}")
print(f"  ✓ Complete. AUC={auc:.4f}  MCC={mcc:.4f}  threshold={OPT_T:.4f}")
